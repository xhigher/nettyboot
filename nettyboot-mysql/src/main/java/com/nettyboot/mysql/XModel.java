...
public abstract class XModel {
    ...
    private final List<ClauseWhere> mWheres = new ArrayList<ClauseWhere>();
    private List<ClauseValue> mValues = new ArrayList<ClauseValue>();
    private final List<Object> mUpdateFields = new ArrayList<>();

    private final List<List<ClauseValue>> mBatchValues = new ArrayList<List<ClauseValue>>();

    private String mStatementSQL = null;
    private final List<Object> mStatementParams = new ArrayList<Object>();
    private final List<List<Object>> mBatchStatementParams = new ArrayList<List<Object>>();
    private final LinkedHashMap<String, Boolean> mOrderList = new LinkedHashMap<>();
    ...
    public boolean insertNotExists() {
        ...
        PreparedStatement pstmt = null;
        prepareStatement(SQLType.INSERT_NOT_EXISTS);
        try {
            pstmt = conn.prepareStatement(mStatementSQL);
            for (int i = 0, n = mStatementParams.size(); i < n; i++) {
                pstmt.setObject(i + 1, mStatementParams.get(i));
    ...
    protected void prepareStatement(SQLType type) {
        ...
        if (type == SQLType.UPDATE) {
        ...
        } else if (type == SQLType.INSERT_NOT_EXISTS) {
            sqlSB.append("INSERT INTO `" + tableName() + "` (");
            ClauseValue clause = null;
            StringBuilder patternSB = new StringBuilder();
            for (int i = 0, n = mValues.size(); i < n; i++) {
            ...
            sqlSB.append(") SELECT " + patternSB.toString());
            sqlSB.append(" FROM DUAL WHERE NOT EXISTS (SELECT 1 FROM `" + tableName() + "` WHERE 1=1 ");
            if (mWheres.size() > 0) {
                ClauseWhere clause2 = null;
                Object value = null;
                for (int i = 0, n = mWheres.size(); i < n; i++) {
                    clause2 = mWheres.get(i);
                    value = clause2.getValue();
                    if (value != null) {
                        mStatementParams.add(value);
                    }
                    sqlSB.append(" AND ");
                    sqlSB.append(clause2.getPattern());
                }
            }
            sqlSB.append(")");
